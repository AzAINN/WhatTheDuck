...
def cmd_benchmark(args: argparse.Namespace) -> None:
    """Direct comparison: classical MC vs quantum AE at ONE threshold."""
    ...
    for n_samples in [100, 500, 1000, 5000, 10000, 50000, 100000]:
        samples = rng.choice(len(probs), size=n_samples, p=probs)
        p_hat = np.mean(samples < threshold_idx)
        err = abs(p_hat - true_p)
        results["classical"].append({"queries": n_samples, "error": err})
        print(f"Classical n={n_samples:>6}: p_hat={p_hat:.6f}, err={err:.6f}")

    # QUANTUM: single threshold estimation at various epsilon
    from qiskit_aer.primitives import Sampler
    sampler = Sampler(run_options={"shots": 1024, "seed": args.seed})
    stateprep = _load_stateprep_qasm(indir / f"{blob['dist_name']}.stateprep.qasm")
    ...
    for eps in [0.05, 0.02, 0.01, 0.005, 0.002, 0.001]:
        ...
        err = abs(est.p_hat - true_p)
        results["quantum"].append({"queries": est.cost_oracle_queries, "error": err, "epsilon": eps})
        print(f"Quantum ε={eps:.3f}: p_hat={est.p_hat:.6f}, err={err:.6f}, queries={est.cost_oracle_queries}")
...
def main() -> None:
    ...
    apb = sub.add_parser("benchmark", help="Compare classical MC vs quantum AE scaling")
    apb.add_argument("--indir", type=str, required=True)
    apb.add_argument("--threshold-idx", type=int, default=8)
    apb.add_argument("--seed", type=int, default=42)
    apb.add_argument("--out", type=str, default="scaling_comparison.json")
    apb.set_defaults(func=cmd_benchmark)
