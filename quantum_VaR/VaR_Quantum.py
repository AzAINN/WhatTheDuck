...
# Module-level defaults (overwritten by setup_distribution or __main__)
grid_points = np.array([])
probs = []


def setup_distribution(grid, pmf, num_qubits=7):
    """Set module-level distribution data for quantum circuits."""
    global grid_points, probs, NUM_QUBITS
    grid_points = np.asarray(grid)
    probs = list(pmf) if not isinstance(pmf, list) else pmf
...
# ---------------------------------------------------------------------------
def iqae_epsilon_sweep(epsilons=None, alpha_fail=IQAE_ALPHA_FAIL,
                       output_path="results/iqae_epsilon_sweep.csv",
                       n_runs=1, eps_min=0.005, eps_max=0.30, n_points=20,
                       use_gpu_warmstart=False, mc_samples=10000, mc_confidence=0.99):
    """
    Sweep IQAE precision (epsilon) and record estimation error vs total cost.
    ...
    epsilons : list[float] | None
        Explicit list of epsilon values.  When None, generates *n_points*
        log-spaced values between *eps_max* and *eps_min* via np.geomspace.
    n_runs : int
        Number of independent IQAE runs per epsilon.  When > 1 the CSV
        contains mean/std columns suitable for error-bar plots.
    use_gpu_warmstart : bool
        If True, run GPU MC warm start before each IQAE to get a bracket.
        Total cost = mc_samples + grover_calls.
    """
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    if epsilons is None:
        # Log-spaced from large to small so coarse (fast) runs come first
        epsilons = np.geomspace(eps_max, eps_min, n_points).tolist()
    ...
    fieldnames = [
        "epsilon", "alpha", "a_true",
        "a_hat_mean", "a_hat_std",
        "abs_error_mean", "abs_error_std",
        "ci_low", "ci_high",
        "mc_samples", "bracket_width_mean",
        "shots_total_mean", "grover_calls_mean",
        "total_samples_mean",  # mc_samples + grover_calls
        "ks_used", "runs",
    ]
    ...
    for eps in epsilons:
        a_hats, abs_errors = [], []
        shots_list, grover_list = [], []
        ci_lows, ci_highs = [], []
        bracket_widths = []
        ks_all = []

        for run_idx in range(n_runs):
            # Optional GPU warm start (just for tracking cost)
            if use_gpu_warmstart:
                ...
                bracket_widths.append(warmup["bracket_width"])

            # Always run IQAE at the fixed reference threshold (not VaR search)
            res = run_iqae_detailed(iqae, epsilon=eps, alpha_fail=alpha_fail)
            est = res["estimation"]
            ci = (res["ci_low"], res["ci_high"])
            shots = res["shots_total"]
            queries = res["grover_calls"]

            a_hats.append(est)
            abs_errors.append(abs(est - a_true))
            shots_list.append(shots)
            grover_list.append(queries)
            ci_lows.append(ci[0])
        ...
        row = {
            ...
            "alpha": alpha_fail,
            "a_true": a_true,
            "a_hat_mean": np.mean(a_hats),
            "a_hat_std": np.std(a_hats, ddof=1) if n_runs > 1 else 0.0,
            "abs_error_mean": np.mean(abs_errors),
            "abs_error_std": np.std(abs_errors, ddof=1) if n_runs > 1 else 0.0,
            "ci_low": np.mean(ci_lows),
            "ci_high": np.mean(ci_highs),
            "mc_samples": mc_samples if use_gpu_warmstart else 0,
            "bracket_width_mean": np.mean(bracket_widths) if bracket_widths else 0,
            "shots_total_mean": np.mean(shots_list),
            "grover_calls_mean": np.mean(grover_list),
            "total_samples_mean": (mc_samples if use_gpu_warmstart else 0) + np.mean(grover_list),
            "ks_used": "",
            "runs": n_runs,
        }
        rows.append(row)
        std_str = f"±{row['a_hat_std']:.6f}" if n_runs > 1 else ""
        warmstart_info = f"  bracket={row['bracket_width_mean']:.1f}" if use_gpu_warmstart else ""
        print(
            f"  eps={eps:>8.5f}  a_hat={row['a_hat_mean']:.6f}"
            f"{std_str:>15s}  "
            f"abs_err={row['abs_error_mean']:.6f}  "
            f"total={row['total_samples_mean']:.0f}{warmstart_info}  (x{n_runs})")
...
""",
    ...
    p.add_argument("--sweep-min", type=float, default=0.005,
                   help="smallest epsilon for sweep (default: 0.005)")
    p.add_argument("--sweep-max", type=float, default=0.30,
                   help="largest epsilon for sweep (default: 0.30)")
    p.add_argument("--sweep-points", type=int, default=20,
                   help="number of log-spaced epsilon values (default: 20)")
    p.add_argument("--sweep-runs", type=int, default=3,
                   help="IQAE runs per epsilon for statistics (default: 3)")
    p.add_argument("--sweep-gpu-warmstart", action="store_true",
                   help="use GPU MC warm start in sweep (tracks mc_samples + grover_calls)")
    p.add_argument("--sweep-mc-samples", type=int, default=10000,
                   help="MC samples for sweep warm start (default: 10000)")
    p.add_argument("--sweep-mc-confidence", type=float, default=0.99,
                   help="MC bracket confidence for sweep (default: 0.99)")
    return p
...
if __name__ == "__main__":
    ...
    if args.sweep:
        print()
        warmstart_label = " with GPU warm start" if args.sweep_gpu_warmstart else ""
        print(f"Running IQAE epsilon sweep{warmstart_label}  "
              f"eps=[{args.sweep_max}..{args.sweep_min}]  "
              f"n_points={args.sweep_points}  runs={args.sweep_runs} ...")
        iqae_epsilon_sweep(
            alpha_fail=args.alpha_fail,
            n_runs=args.sweep_runs,
            eps_min=args.sweep_min,
            eps_max=args.sweep_max,
            n_points=args.sweep_points,
            use_gpu_warmstart=args.sweep_gpu_warmstart,
            mc_samples=args.sweep_mc_samples,
            mc_confidence=args.sweep_mc_confidence,
        )
